			...
			success: function(data) {
										...
										var result = '', sort_up = '', sort_down='', edit='', del='';
											for(var i=0; i<data.rows.length; i++){												
												...
												result += '<tr class="odd gradeX"><td>'+(i+1)+'</td><td>'+data.rows[i]['name_privilege']+'</td><td>'+data.rows[i]['name']+'</td><td>'+data.rows[i]['url']+'</td><td align="center"><span class="'+data.rows[i]['icon']+'"></span></td><td>'+data.rows[i]['status']+'</td><td>'+access_view+access_save+access_edit+access_delete+'</td><td>'+sort_up+' '+sort_down+'</td><td>'+edit+' '+del+'</td></tr>';
											}
